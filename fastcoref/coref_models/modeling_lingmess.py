...
class LingMessModel(BertPreTrainedModel):
    _tied_weights_keys: typing_ClassVar = {}
    all_tied_weights_keys: typing_ClassVar = {}
    ...
    def __init__(self, config):
        ...
        self.start_mention_mlp = FullyConnectedLayer(
            config, self.hidden_size, self.ffnn_size, self.dropout_prob
        )
        ...
        self.coref_start_all_mlps = FullyConnectedLayer(
            config, config.hidden_size, self.all_cats_size, self.dropout_prob
        )
        self.coref_end_all_mlps = FullyConnectedLayer(
            config, config.hidden_size, self.all_cats_size, self.dropout_prob
        )

        self.antecedent_s2s_all_weights = nn.Parameter(
            torch_empty((self.num_cats, self.ffnn_size, self.ffnn_size))
        )
        ...
        self.antecedent_s2s_all_biases = nn.Parameter(
            torch_empty((self.num_cats, self.ffnn_size))
        )
        self.antecedent_e2e_all_biases = nn.Parameter(
            torch_empty((self.num_cats, self.ffnn_size))
        )
        self.antecedent_s2e_all_biases = nn.Parameter(
            torch_empty((self.num_cats, self.ffnn_size))
        )
        self.antecedent_e2s_all_biases = nn.Parameter(
            torch_empty((self.num_cats, self.ffnn_size))
        )
    ...
    def _get_clusters_labels(self, span_starts, span_ends, all_clusters):
        ...
        for b, (starts, ends, gold_clusters) in enumerate(
            zip(span_starts_cpu, span_ends_cpu, all_clusters_cpu, strict=False)
        ):
    ...
    def _get_categories_labels(
        ...
        spans = []
        for b, (starts, ends) in enumerate(
            zip(span_starts.cpu().tolist(), span_ends.cpu().tolist(), strict=False)
        ):
            doc_spans = []
            for start, end in zip(starts, ends, strict=False):
                token_indices = [
                    new_token_map[b].get(idx, False)
                    for idx in set(subtoken_map[b][start : end + 1]) - {False}
                ]
                span = {
                    tokens[b][idx].lower() for idx in token_indices if idx is not None
                }
        ...
        for b in range(batch_size):
            for i in range(max_k):
                for j in list(range(max_k))[:i]:
                    categories_labels[b, i, j] = get_category_id(
                        spans[b][i], spans[b][j]
                    )

        categories_labels = torch_tensor(categories_labels, device=self.device)
        categories_masks = [
            categories_labels == cat_id for cat_id in range(self.num_cats - 1)
        ] + [categories_labels != -1]
    ...
    def _get_marginal_log_likelihood_loss(self, logits, labels, span_mask):
        ...
        )  # [batch_size, num_cats + 1, max_k]
        all_log_sum_exp = torch_logsumexp(
            logits, dim=-1
        )  # [batch_size, num_cats + 1, max_k]
    ...
    def _calc_coref_logits(self, start_reps, end_reps):
        ...
        biases = (
            torch_einsum(
                "bnkf, nf -> bnk", all_starts, self.antecedent_s2s_all_biases
            ).unsqueeze(-2)
            + torch_einsum(
                "bnkf, nf -> bnk", all_ends, self.antecedent_e2e_all_biases
            ).unsqueeze(-2)
            + torch_einsum(
                "bnkf, nf -> bnk", all_ends, self.antecedent_s2e_all_biases
            ).unsqueeze(-2)
            + torch_einsum(
                "bnkf, nf -> bnk", all_starts, self.antecedent_e2s_all_biases
            ).unsqueeze(-2)
        )
    ...
    def _get_all_labels(self, clusters_labels, categories_masks):
        ...
        categories_labels = (
            clusters_labels.unsqueeze(1).repeat(1, self.num_cats, 1, 1)
            * categories_masks
        )
        ...
        # null cluster
        zeros = torch_zeros(
            (batch_size, self.num_cats + 1, max_k, 1), device=self.device
        )
        all_labels = torch_cat(
            (all_labels, zeros), dim=-1
        )  # [batch_size, num_cats + 1, max_k, max_k + 1]
    ...
    def forward_transformer(self, batch):
        ...
        if "leftovers" not in batch:
            ...
            if len(leftovers_ids) > 0:
                res_outputs = self.base_model(
                    leftovers_ids, attention_mask=leftovers_mask
                )
                res_sequence_output = res_outputs.last_hidden_state

                attention_mask = torch_cat([attention_mask, leftovers_mask], dim=1)
                sequence_output = torch_cat(
                    [sequence_output, res_sequence_output], dim=1
                )
    ...
    def forward(self, batch, gold_clusters=False, return_all_outputs=False):
        ...
        # gather reps
        topk_start_reps = torch_gather(
            sequence_output, dim=1, index=mention_start_ids.unsqueeze(-1).expand(size)
        )
        topk_end_reps = torch_gather(
            sequence_output, dim=1, index=mention_end_ids.unsqueeze(-1).expand(size)
        )
