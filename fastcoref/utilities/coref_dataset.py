...
def _tokenize(tokenizer, tokens, clusters, speakers):
    ...
    encoded_text = tokenizer(
        new_tokens, add_special_tokens=True, is_split_into_words=True, return_length=True, return_attention_mask=False
    )
...
def encode(example, tokenizer, nlp):
    if "tokens" in example and example["tokens"]:
        ...
        new_clusters = [
            [(spacy_doc.char_span(start, end).start, spacy_doc.char_span(start, end).end - 1) for start, end in cluster]
            for cluster in clusters
        ]
        # verify alignment
...
def create(file, tokenizer, nlp):
    ...
    dataset = dataset.map(encode, batched=False, fn_kwargs={"tokenizer": tokenizer, "nlp": nlp}, load_from_cache_file=True)
