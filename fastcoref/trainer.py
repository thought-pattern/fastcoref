...
def _load_f_coref_model(args):
    ...
    logger.info(f"FCoref Parameters: {t_params + h_params:.1f}M, " f"Transformer: {t_params:.1f}M, Coref head: {h_params:.1f}M")
...
class CorefTrainer:
    def __init__(self, args: TrainingArgs, train_file, dev_file=None, test_file=None, nlp=None):
        ...
        self.nlp = (
            ...
            else spacy.load("en_core_web_sm", exclude=["tagger", "parser", "lemmatizer", "ner", "textcat"])
        )

        self.model, self.tokenizer = _load_f_coref_model(self.args)
        self.model.to(self.device)

        self.collator = LeftOversCollator(tokenizer=self.tokenizer, device=self.device, max_segment_len=self.args.max_segment_len)
