...
class LeftOversCollator:
    ...
    def __call__(self, batch):
        ...
        input_ids = [
            [ids[i : i + self.max_segment_len] for i in range(0, len(ids), self.max_segment_len)]
            for ids in batch.get("input_ids", [])
        ]
        attention_mask = [
            [mask[i : i + self.max_segment_len] for i in range(0, len(mask), self.max_segment_len)]
        ...
        if len(input_ids[0]) > 1 and len(input_ids[0][-1]) < self.max_segment_len:
            batch.get("leftovers", {})["input_ids"] = torch_tensor([ids[-1] for ids in input_ids], device=self.device)
            batch.get("leftovers", {})["attention_mask"] = torch_tensor([mask[-1] for mask in attention_mask], device=self.device)
        ...
        max_num_clusters, max_max_cluster_size = (
            max(1, max(batch.get("num_clusters", []))),
            max(1, max(batch.get("max_cluster_size", []))),
        )
        padded_clusters = [
            pad_clusters(cluster, max_num_clusters, max_max_cluster_size) for cluster in batch.get("gold_clusters", [])
        ]
        batch["gold_clusters"] = torch_tensor(padded_clusters, device=self.device)
...
class PadCollator:
    ...
    def __call__(self, batch):
        ...
        batch["attention_mask"] = torch_tensor(batch.get("attention_mask", []), device=self.device)
...
class DynamicBatchSampler:
    ...
    def calc_effective_per_example_batch_len(self, example_len):
        computed_return_value = math_ceil(example_len / self.max_segment_len) * self.max_segment_len
