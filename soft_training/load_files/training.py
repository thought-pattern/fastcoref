...
def train(args, train_batches, model, tokenizer, evaluator):
    ...
    optimizer = AdamW(
        optimizer_grouped_parameters, lr=args.learning_rate, betas=(args.adam_beta1, args.adam_beta2), eps=args.adam_epsilon
    )
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            teacher_coref_logits = torch.from_numpy(
                np.stack([np.load(os.path.join(teacher_logits_dir, k + "_coref_logits.npy")) for k in keys], axis=0)
            ).to(args.device)
            topk_1d_indices = torch.from_numpy(
                np.stack([np.load(os.path.join(teacher_logits_dir, k + "_top_indices.npy")) for k in keys], axis=0)
            ).to(args.device)
            model.zero_grad()
            model.train()
    ...
    with open(os.path.join(args.output_dir, "best_f1.json"), "w") as f:
