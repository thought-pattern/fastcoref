        ...
        for _step, batch in enumerate(epoch_iterator):
            ...
            teacher_coref_logits = torch_from_numpy(
                np_stack(
                    [
                        np_load(
                            os_path.join(teacher_logits_dir, k + "_coref_logits.npy")
                        )
                        for k in keys
                    ],
                    axis=0,
            ...
            topk_1d_indices = torch_from_numpy(
                np_stack(
                    [
                        np_load(
                            os_path.join(teacher_logits_dir, k + "_top_indices.npy")
                        )
                        for k in keys
                    ],
                    axis=0,
            ...
            with torch_cuda.amp.autocast():
                outputs = model(
                    batch, topk_1d_indices=topk_1d_indices, return_all_outputs=True
                )
                span_mask = outputs[0]
